...
# Step 6: Train XGBoost model with more regularization
xgb = XGBRegressor(
    n_estimators=300,
    learning_rate=0.05,
    max_depth=6,
...
# === NEW PART: Read sensor data from ThingSpeak ===
def read_sensor_data():
    channel_id = '2958025'
    read_api_key = 'I8R19U2FG4Q5O9TY'
    url = f"https://api.thingspeak.com/channels/{channel_id}/feeds.json"
    params = {'api_key': read_api_key, 'results': 5}  # Get last 5
    ...
    try:
        feeds = response.json()['feeds'][::-1]  # Reverse to check latest first
        for feed in feeds:
            if all(feed[f'field{i}'] is not None for i in range(1, 5)):
                sensor_data = {
                    ...
                    'soil': 'loamy',  # Assuming soil type is constant for simplicity
                    'label': 'normal'  # Assuming label is constant for simplicity
                }
                return sensor_data
...
# === Simple Outlier Handling Function ===
def handle_outliers(data, min_val, max_val):
    return np.clip(data, min_val, max_val)

# === Predict and send NPK to ThingSpeak ===
